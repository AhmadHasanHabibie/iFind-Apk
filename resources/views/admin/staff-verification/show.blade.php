        ...
        <div class="p-6 sm:p-8 bg-gradient-to-r from-slate-900 to-slate-800 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center space-x-3">
                <form method="POST" action="{{ route('admin.chat.start', $user) }}">
                    @csrf
                    <button type="submit" class="px-4 py-2.5 bg-teal-600/90 hover:bg-teal-600 text-white font-bold text-xs rounded-xl shadow-sm transition flex items-center space-x-1.5 border border-teal-500/40">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                        <span>Chat Staf Ini</span>
                    </button>
                </form>

                @if($user->verification_status !== 'approved')
                    <form method="POST" action="{{ route('admin.staff-verification.approve', $user) }}" data-confirm="Apakah Anda yakin ingin menyetujui akun staf ini?" data-confirm-title="Setujui Akun Staf" data-confirm-btn="Ya, Setujui Akun">
                        @csrf
