...
<head>
    ...
    <title>Masuk — i-Find Platform Reservasi & Spot Nongkrong</title>
    <!-- Tailwind CSS CDN -->
    ...
    <div class="w-full max-w-md bg-white rounded-3xl border border-slate-100 p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] relative z-10">
        ...
        <div class="flex flex-col items-center text-center">
            ...
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">
                Selamat Datang di <span class="text-blue-600">i-Find</span>
            </h1>
            <p class="text-sm text-slate-500 mt-2 mb-6">
                Masuk ke akun Anda untuk menemukan spot terbaik
            </p>
        </div>

        <!-- Flash Success Notification -->
        @if (session('success'))
            <div class="mb-5 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs leading-relaxed flex items-center gap-2">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-4">
            ...
            <div class="flex items-center pt-1">
                <input id="remember_me" 
                       name="remember" 
                       type="checkbox" 
                       class="w-4 h-4 text-blue-600 bg-slate-100 border-slate-300 rounded focus:ring-blue-500/20 focus:ring-2 cursor-pointer">
