...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-2">
            @if($ticket->status === 'open')
                <span class="px-3.5 py-1.5 rounded-full text-xs font-bold bg-amber-50 text-amber-600 border border-amber-200">
                    <i class="fa-solid fa-clock mr-1"></i> Menunggu Tindak Lanjut Admin
                </span>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 p-6 shadow-xs space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-900">{{ auth()->user()->name }} (Staf Toko)</p>
    ...
    <!-- Replies Thread -->
    <div class="space-y-4">
        <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Diskusi & Respon Admin ({{ $ticket->replies->count() }})</h3>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                @if($isAdmin)
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-md bg-teal-100 text-teal-700">Administrator iFind</span>
                                @endif
                            </div>
        ...
        @empty
            <div class="bg-slate-50 rounded-2xl p-6 text-center text-xs text-slate-400">
                Belum ada balasan dari tim admin. Tiket Anda sedang dalam antrean penanganan.
            </div>
        @endforelse
    </div>
    ...
    @if($ticket->status !== 'closed')
        <form method="POST" action="{{ route('staff.tickets.reply', $ticket) }}" class="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
            @csrf
            <div>
                <label for="message" class="block text-xs font-bold text-slate-700 mb-1.5">Tulis Balasan ke Admin</label>
                <textarea id="message" name="message" rows="3" required
                          placeholder="Tulis informasi tambahan atau jawaban..."
