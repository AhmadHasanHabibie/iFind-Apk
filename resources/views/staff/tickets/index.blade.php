...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
        <div>
            <h1 class="text-2xl font-black text-slate-900 flex items-center gap-2.5">
                ...
                Tiket Pengaduan Staf
            </h1>
            <p class="text-sm text-slate-500 mt-1">Sampaikan kendala operasional, pembayaran toko, atau permohonan bantuan ke tim Admin iFind.</p>
        </div>
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-2">
        ...
        <a href="{{ route('staff.tickets.index', ['status' => 'open']) }}"
        ...
        <a href="{{ route('staff.tickets.index', ['status' => 'in_progress']) }}"
        ...
        <a href="{{ route('staff.tickets.index', ['status' => 'resolved']) }}"
        ...
        <a href="{{ route('staff.tickets.index', ['status' => 'closed']) }}"
    ...
    @if($tickets->isEmpty())
        <div class="bg-white rounded-3xl border border-slate-100 p-12 text-center shadow-xs">
            ...
            <p class="text-xs text-slate-500 max-w-sm mx-auto mt-1">Jika toko Anda memerlukan verifikasi manual atau mengalami kendala teknis, buatlah tiket pengaduan baru.</p>
            <a href="{{ route('staff.tickets.create') }}" class="inline-flex items-center mt-5 px-4 py-2 rounded-xl bg-blue-600 text-white font-bold text-xs hover:bg-blue-700 transition">
                <i class="fa-solid fa-plus mr-1.5"></i> Buat Tiket Staf
            </a>
        </div>
    @else
        <div class="bg-white rounded-3xl border border-slate-100 shadow-xs divide-y divide-slate-100 overflow-hidden">
            @foreach($tickets as $ticket)
                <a href="{{ route('staff.tickets.show', $ticket) }}" class="block p-5 hover:bg-slate-50/80 transition group">
