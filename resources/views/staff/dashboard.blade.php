...
<div class="space-y-6">
    ...
    @else
        ...
        @if(! $store->canAcceptBookings())
            <div class="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                    </div>
                    <div>
                        <h4 class="text-sm font-extrabold text-amber-900">Perhatian: Pengaturan Pembayaran Belum Lengkap</h4>
                        <p class="text-xs text-amber-700 mt-0.5">Toko Anda belum bisa menerima booking. Lengkapi pengaturan harga & pembayaran.</p>
                    </div>
                </div>
                <a href="{{ route('staff.store.edit') }}" class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl transition shrink-0 shadow-sm">
